...
ЗНАК ПЕРЕД ЧИСЛОМ МЕНЯЕТ СМЫСЛ — НЕ ИГНОРИРУЙ ЕГО. Тире, дефис и минус (- / — / –)
перед числом ВСЕГДА значат ВЫЧЕСТЬ. Это "add" с ОТРИЦАТЕЛЬНОЙ суммой, а НЕ новый
остаток. НЕ спрашивай, новый это остаток или вычесть — тире это минус, точка.
- «Рабочий баланс - 363 985» / «Нико - 95 500» / «Нико минус 95 500» / «Нико -95500» /
  «у Нико забрали 95 500» / «Нико убыло на 95 500» → add с amount -363985 / -95500.
  ЭТО НЕ set и это НЕ повод для question.
- «Нико + 20 000» / «Нико добавили 20 000» / «Нико плюс 20 000» → add +20000.
- «Нико 95 500» БЕЗ знака и без слов про изменение → set (новый остаток), как обычно.
Про минус/тире question НЕ задавай никогда — знак однозначен.
...
- НОВАЯ сумма, которую называет Илья, дана в рублях (реестр долларовый) — спроси,
  сколько это в долларах.
ВЕСЬ РЕЕСТР — В ДОЛЛАРАХ. Всегда, без исключений. Он НЕ в рублях. Знак $ у чисел
в ТЕКУЩЕМ РЕЕСТРЕ — это доллары, а не признак, что что-то не так. НИКОГДА не пиши
«реестр вроде в рублях», не спрашивай «а 698 000 это рубли?» и не сомневайся в
валюте того, что уже лежит в реестре, — бери эти значения как доллары.
Вопрос про рубли уместен ТОЛЬКО когда Илья называет НОВУЮ сумму словом «рублей»
или «₽». И не мешай в один question две разные вещи.
...
# Одна строка — только деньги: где лежит, что потрачено, что менялось.
# Остальное (сводки, список, поиск, удаление) живёт в меню команд «/» и голосом —
# Илья попросил не загромождать клавиатуру.
...
async def _post_init(app: Application) -> None:
    ...
    # Меню — только деньги, по просьбе Ильи. Остальные обработчики живы (старые
    # привычки и закешированные кнопки не сломаются), но наружу не рекламируются.
